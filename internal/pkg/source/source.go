...
func identify(pass *analysis.Pass, stc config.SourceTypeClassifier, ssaInput *buildssa.SSA) map[*ssa.Function][]*Source {
	sourceMap := make(map[*ssa.Function][]*Source)

	for _, fn := range ssaInput.SrcFuncs {
		var sources []*Source
		sources = append(sources, sourcesFromParams(pass, fn, stc)...)
		sources = append(sources, sourcesFromClosure(pass, fn, stc)...)
		sources = append(sources, sourcesFromBlocks(pass, fn, stc)...)
...
func sourcesFromParams(pass *analysis.Pass, fn *ssa.Function, stc config.SourceTypeClassifier) []*Source {
	var sources []*Source
...
func sourcesFromClosure(pass *analysis.Pass, fn *ssa.Function, stc config.SourceTypeClassifier) []*Source {
	var sources []*Source
...
func sourcesFromBlocks(pass *analysis.Pass, fn *ssa.Function, stc config.SourceTypeClassifier) []*Source {
	var sources []*Source
